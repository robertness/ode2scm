...
Here are some rates and initial states that we are going to use for the MAPK model.  Note that the initial states determine the range of possible values at any time point including after steady state.  For example, the sum of Mek, and PMek is constant.  

It is important that the steady states are not close to the boundary values.  For example, the steady state of PMek should not be 100 or too close to 100 if Mek + PMek == 100.  Otherwise the formulation of the SCM we are using will not work.  So when running sensitivity tests, you want to check the steady state values you get are not on the boundaries.
...
lines(times, ode_out$PMek, type='l', col='darkorange')
lines(sde_out$PMek, type='l', col='darkorange', lty=3)
abline(h = Mek, col='darkorange', lty=2)

lines(times, ode_out$PPrk, type='l', col='darkred')
lines(sde_out$PErk, type='l', col='darkred', lty=3)
...
for rate_set in rate_sets:
  # With ODE model
  run ODE with rate_set, define raf0, mek0, erk0 as steady states of PRaf, PMek, and PErk
  change the Raf activation rate to a valid new number
  run the ODE with updated rate set, define raf1, erk1 as steady state PRaf/PErk
...
source_python(system.file("python", "mapk_scm_no_double_phos.py", package = "ode2scm"))
...
```
  ...
  for rate_set in rate_sets:
    ...
    for seed in seeds:
      set_seed(seed)
      run SDE with rate_set, define raf0, mek0, erk0 as steady states of PRaf, PMek, and PErk
      change the Raf activation rate to a valid new number
      run the ODE with updated rate set, define raf1 as ODE steady state for PRaf
      run the SDE with updated rate set, define erk1 as SDE steady state for PErk
